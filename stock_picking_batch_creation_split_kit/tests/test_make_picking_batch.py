...
class TestMakePickingBatch(ClusterPickingCommonFeatures):
    ...
    def setUpClass(cls):
        ...
        # Add the kits to the picking
        cls._add_product_to_picking(cls.pick1, cls.product_table)
        ...
        cls.make_picking_batch = cls.makePickingBatch.create(
            {
                ...
                "stock_device_type_ids": [
                    Command.link(cls.device1.id),
                    Command.link(cls.device2.id),
                    Command.link(cls.device3.id),
                ],
