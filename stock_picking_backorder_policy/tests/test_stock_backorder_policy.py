...
class TestStockBackorderPolicy(BackorderPolicyCommon):
    ...
    def test_no_policy_falls_back_to_operation_type(self):
        ...
        result = picking._check_backorder()
        self.assertTrue(
            result, "With no policy, the backorder wizard should be triggered"
        )
    ...
    def test_policy_always(self):
        ...
        # Wizard should be suppressed
        result = picking._check_backorder()
        self.assertFalse(result, "With 'always' policy, wizard should be suppressed")

        # Complete validation — should create a backorder
        ...
        self.assertTrue(backorder, "A backorder should have been auto-created")
    ...
    def test_policy_never(self):
        ...
        result = picking._check_backorder()
        self.assertFalse(result, "With 'never' policy, wizard should be suppressed")

        picking._action_done()

        # No backorder should exist
        backorder = self.env["stock.picking"].search(
            [
                ("backorder_id", "=", picking.id),
            ]
        ...
        self.assertFalse(
            backorder, "No backorder should have been created for 'never' policy"
        )
    ...
    def test_policy_ask(self):
        ...
        result = picking._check_backorder()
        self.assertTrue(
            result, "With 'ask' policy, backorder wizard should still be triggered"
        )
    ...
    def test_full_delivery_no_backorder_check(self):
        ...
        self.assertFalse(backorder, "No backorder expected for full delivery")
