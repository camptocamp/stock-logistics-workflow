...
class TestStockBackorderPolicy(BackorderPolicyCommon):
    ...
    def test_policy_propagates_through_mto_chain(self):
        ...
        # A pull rule that replenishes the output location from stock, procured
        # on order, so confirming an outgoing move generates the chained
        # upstream (pick) move via procurement.
