...
class TestBatchPicking(SavepointCase):
    ...
    def test_partial_done(self):
        ...
        inmediate_transfer_action = (
            Form(self.env["stock.immediate.transfer"].with_context(context))
            .save()
            .process()
        )
        backorder_confirmation_context = inmediate_transfer_action.get("context")
        # Create backorder? action
        Form(
            self.env["stock.backorder.confirmation"].with_context(
                backorder_confirmation_context
            )
        ).save().process()
        self.batch.remove_undone_pickings()
