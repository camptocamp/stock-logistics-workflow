...
class TestStockPickingGroupByPartnerAtShipping(SavepointCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.env = cls.env(context=dict(cls.env.context, tracking_disable=True))
        # Two customer one is invoice at shipping the other one not
        cls.partner1 = cls.env.ref("base.res_partner_1")
        cls.partner1.invoicing_mode = "at_shipping"
        cls.partner2 = cls.env.ref("base.res_partner_2")
        cls.product = cls.env.ref("product.product_delivery_01")
        # Create one sales order each, but both delivered to the same customer
    ...
    def check_only_one_sale_is_invoiced(self):
        """Check one picking with 2 sales order only one invoiced at shipping."""
